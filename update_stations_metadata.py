...
for s in stations:
    ...
    if num_match:
        ...
        if store_num in tank_mappings:
            ...
            # Check if any tank was actually matched
            if not s["tanks_info"]:
                unmatched.append((s["id"], "No tank sheets matched"))
        else:
            unmatched.append((s["id"], "Store number not found in HTML"))
...
print("\nSuccessfully updated stations.json with tank descriptions!")
